# Code adapted from KidsCanCode Shmup by Danielle Ravenwing 2018
# Feel free to use or edit this code for your own use as long as your credit my work.
# Artwork by Danielle Ravenwing and Kenny on OpenGameArt
...
def show_start_screen(surf):
    ...
    draw_text(screen, "Use Escape to quit, arrow keys to move, P to Pause, left shift to fire, and Z for antimatter bombs.", 22, WIDTH / 2, HEIGHT / 2)
        ...
        if (now - self.last_shot > self.shoot_delay) and self.energy > 0:
            ...
            for i in range(self.power):
                ...
                if self.energy < 0:
                    self.energy = 0
                if self.power > 1:
                    bullet = Bullets(self.rect.centerx, self.rect.top, self.speedx, self.speedy, int(-30 * math.sin(math.radians(i * 10))), int(-10 * math.cos(math.radians(i * 10))), i * 10)
...
class Black_Hole(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.size = random.randrange(WIDTH/3.2, WIDTH/2)
...
class Bullets(pygame.sprite.Sprite):
    def __init__(self, x, y, xspeed, yspeed, bulletx_speed = 0, bullety_speed = -30, angle = 0):
        pygame.sprite.Sprite.__init__(self)
